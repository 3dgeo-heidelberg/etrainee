...
To be able to determine the best scale of elaboration, studies that tested different spatial resolutions within the same area and used classification legends with different numbers of categories are important. When we compared data with different spatial resolutions classified for the same area in the Krkonoše Mts, 8 categories (six grass species and two other vegetation categories) were distinguishable from UAV HS and MS data (see *Figure 5* – results for UAV data) with the pixel size 3 cm/pixel. While the legend had to be generalized for satellite and aerial data with larger pixels. It was not possible to distinguish some less abundant grass categories and individual trees/shrubs in their pixels.
